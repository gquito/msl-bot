#cs ----------------------------------------------------------------------------
 ...
 Compares color variation with defined pixel and pixel from BlueStacks

 Parameters:

	arrayPixel - Coordinates and Color.

	intVariation - Variation limit.

 Returns:

	Within variation - Return True

	Not within variation - Return False

#ce ----------------------------------------------------------------------------

Func checkPixel($arrayPixel, $intVariation = 10)
	Return _ColorCheckVariation("0x" & Hex(_GDIPlus_BitmapGetPixel($hBitmap, $arrayPixel[0], $arrayPixel[1]), 6), "0x" & Hex($arrayPixel[2], 6), $intVariation)
EndFunc

#cs ----------------------------------------------------------------------------

 Function: checkPixelWait

 Checks pixel and wait a certain duration

 Parameters:

	arrayPixel - Coordinates and Color.

	intDuration - Time to wait and check for.
...
Func checkPixelWait($arrayPixel, $intDuration = 5, $intVariation = 10)
	$intDuration = $intDuration * 1000
	$startTime = TimerInit()
	While TimerDiff($startTime) < $intDuration
		If _Sleep(100) Then Return
		_CaptureRegion()
		If _ColorCheckVariation("0x" & Hex(_GDIPlus_BitmapGetPixel($hBitmap, $arrayPixel[0], $arrayPixel[1]), 6), "0x" & Hex($arrayPixel[2], 6), $intVariation) Then
			Return True
		EndIf
	WEnd
	Return False
