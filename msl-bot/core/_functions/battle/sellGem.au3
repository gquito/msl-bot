	...
	Else ;not egg
		...
		Switch $arrayData[0]
			...
			Case "EGG"
				...
				clickUntil($battle_coorSellCancel, "battle-end")
				setLog("Grade: Egg |Shape: - |Type: - |Stat: - |Substat: -", 1)
				Return $arrayData
			Case Else
				setLog("Could not filter gem. Keeping this gem.", 2)
				Return sellGem("!", 0, 0)
		EndSwitch
...
Func getStone()
	...
	For $stone In $stoneIndex
		If checkPixelRecord(getPixelRecord($stone), "392, 162") > 95 Then
			...
			$arrayData[2] = ""
			For $i = 1 To 5
				If checkPixelRecord(getPixelRecord($stone & ";" & $i), "460, 218") > 95 Then
					$arrayData[2] = $i
					ExitLoop
				EndIf
			Next
			...
			If $arrayData[2] = "" Then
				Local $quantity = getCombo("1,2,3,4,5", "3")
				recordPixel($stone & ";" & $quantity, "460, 218", 20, 10)
				$arrayData[2] = $quantity
			EndIf
	...
	Switch $arrayData[0]
		Case "NORMAL", "FIRE", "WATER", "WOOD", "LIGHT", "DARK"
			clickUntil($battle_coorSellCancel, "battle-end")
