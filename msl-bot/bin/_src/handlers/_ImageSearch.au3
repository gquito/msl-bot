...
Func ProcessImageReturn($aResult, $iLeft, $iTop, $bCenter, $sImage, $bMultiple = False)
    ...
    Else
        ...
        If ($aSplitResult[0] = "0" Or $aSplitResult[0] = "") Then Return 0 ; Not Found

        If (Ubound($aSplitResult) < 5) Then 
            Log_Add("Error in array size: 0=" &$aSplitResult[0] & ", Ubound=" & Ubound($aSplitResult),$LOG_ERROR)
            Return -5
        EndIf
...
Func _FindImage($sSourcePath = "", $sImage = "", $iTolerance = 95, $iLeft = 0, $iTop = 0, $iWidth = 800, $iHeight = 552, $bUseColor = False, $bMultiple = False)
    ...
    If ($sSourcePath = "") Then
        $aResult = DllCall($hImageSearch, "wstr:cdecl", "FindImageEX", "handle", $g_hHBitmap, "int", $iWidth, "int", $iHeight, "str", $sImage, "int", $iTolerance, "bool", Not($bUseColor), "bool", True, "bool", True)
    Else
        $aResult = DllCall($hImageSearch, "wstr:cdecl", "FindImage", "str", @ScriptDir & $sSourcePath, "str", $sImage, "int", $iTolerance, "bool", Not($bUseColor), "bool", True, "bool", True)
    EndIf
